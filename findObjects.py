...
# Capture image
def capImage():
    cameraCapture = cv2.VideoCapture(1) # open usb camera
...
def detectWheels(img):
    ...
    for i in range(len(qr)):    
        ...
        wheel_mid_points[int(qr[i][0])] = pt_mid
...
def detectCBox(img):
    # Preprocessing--------------------------------------------------------------------
    imgsub = img[:600,400:1400]
    imgflt = cv2.bilateralFilter(imgsub,9,75,75)
    # Pick object indices---------------------------------------------------------------
    ind = np.argwhere(imgflt>200)
    indf = np.flip(ind,1)
    # OBB------------------------------------------------------------------------------
...
def detectBat(img):
    # Preprocessing--------------------------------------------------------------------
    imgsub = img[600:,300:1300]
    imgflt = cv2.bilateralFilter(imgsub,9,75,75)
    # Pick object indices---------------------------------------------------------------
    ind = np.argwhere(imgflt>150)
    indf = np.flip(ind,1)
    # OBB------------------------------------------------------------------------------
    ca = np.cov(indf,y = None,rowvar = 0,bias = 0)
    v, vect = np.linalg.eig(ca)
    ...
    # Coords convertion
    bat_center[0] = bat_center[0]+300
    bat_center[1] = bat_center[1]+600
    bat_center[0] = (bat_center[0] - 1920/2)*12.8/1920*558/5
    bat_center[1] = -(bat_center[1] - 1080/2)*12.8/1920*558/5 + 98
